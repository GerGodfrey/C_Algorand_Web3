...
def game():
    ...
    '''
    monsterHealth = Btoi(Txn.application_args[0]) #Byte to Int 

    # Creamos el mounstruo 
        #Assert = A control flow expression to verify that a condition is true.
    handle_creation = Seq([
    ...
    '''
    handle_optin = Seq([
        Assert(App.optedIn(Txn.sender(), Txn.application_id())), #Compruebe si una cuenta ha optado por una aplicación. 
    ...
    reward_player = Seq([
        Assert(Txn.sender() == Global.creator_address()), #solo el creador puede mandar a llamar esta funcion
        Assert(currentMonsterHealth <= Int(0)),
        ...
        InnerTxnBuilder.Submit(), # Se mandaan 
        Return(Int(1))
    ])
